...
def ask_choice(prompt: str, choices: Dict[str, object], default: Optional[str] = None):
    while True:
        ...
        if not raw and default is not None and default in choices:
...
def validate_yyyymmdd(value: str) -> bool:
    try:
...
def compute_dorking_ui_mark(dorking_flag: str) -> str:
    try:
        ...
        if dorking_flag in mapping:
            db_name, table = mapping[dorking_flag]
...
def process_report(options: ScanOptions):
    with console.status("[magenta]Processing scan...[/magenta]", spinner="dots"):
        ...
        keywords_flag = 1 if options.page_search and options.keywords else 0
    ...
    if options.report_type == ReportType.HTML:
        html_report_creation.report_assembling(
            options.short_domain,
            options.url,
            options.case_comment,
            data_array,
            report_info_array,
            options.pagesearch_ui_mark,
            end_time_str,
            options.snapshotting_ui_mark,
        )
...
def handle_scan():
    ...
    if page_search:
        ...
        if keywords_input.lower() != "n":
            keywords_list = [item.strip() for item in keywords_input.split(",") if item.strip()]
    ...
    if dorking_raw in {"basic", "iot", "files", "admins", "web"}:
        ...
        try:
            custom_db_name = sanitize_db_filename(
                input(Fore.YELLOW + "Enter your custom Dorking DB name (no extension) >> ").strip()
            )
    ...
    if api_yes:
        ...
        if "3" in used_api_ids:
            user_input = input(
                Fore.YELLOW + "If you know some username from this domain, please enter it here (or N if not) >> "
            ).strip()
            username = None if user_input.lower() == "n" else user_input
    ...
    snap_choice = ask_choice(
        ...
        {
            "s": SnapshotMode.SCREENSHOT,
            "p": SnapshotMode.PAGE_COPY,
            "w": SnapshotMode.WAYBACK,
            "n": SnapshotMode.NONE,
        },
        default="n",
    )
    ...
    cli_init.print_prescan_summary(
        short_domain,
        report_type.value.upper(),
        pagesearch_ui_mark,
        dorking_ui_mark,
        used_api_ui,
        case_comment,
        snapshotting_ui_mark,
    )
...
def handle_settings():
    ...
    if choice_settings == "1":
        ...
        with JOURNAL_PATH.open("w", encoding="utf-8"):
            pass
...
def handle_dorking_db():
    ...
    if choice_dorking == "1":
        ...
        try:
            ddb_name = sanitize_db_filename(
                input(Fore.YELLOW + "Enter a name for your custom Dorking DB (no extension) >> ").strip()
            )
        except ValueError as e:
...
def handle_db_menu():
    ...
    if choice_db == "1":
        ...
        _, _, data_presence_flag = db.db_select()
        if data_presence_flag:
            try:
                ...
                print(
                    Fore.RED
                    + "Report with the same recreated folder already exists. Please check its content or delete it and try again"
                    + Style.RESET_ALL
                )
            except Exception as e:
...
def handle_docs():
    try:
        ...
        print(Fore.RED + "Failed to open documentation in browser" + Style.RESET_ALL)
        logging.error("DOCS OPEN ERROR: %s", e, exc_info=True)
...
def run():
    while True:
        try:
            ...
            print(Fore.RED + "\nDPULSE process was ended by user" + Style.RESET_ALL)
            sys.exit()
