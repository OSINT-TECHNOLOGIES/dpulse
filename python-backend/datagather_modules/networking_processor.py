...
def get_dns_info(short_domain, report_file_extension):
    try:
        ...
        if report_file_extension in ('xlsx', 'pdf', 'html'):
        ...
        print(Fore.RED + "Error while getting MX records. See journal for details")
        logging.error(f'DNS INFO GATHERING: ERROR. REASON: {error_other}')
        return 'No information about MX records was gathered'
